/* Content script for Super Highlighter - Chrome Version */

// Mark content script as loaded with timestamp
window.superHighlighterLoaded = true;
window.superHighlighterLoadTime = Date.now();

// Check chrome API availability
if (typeof chrome === 'undefined') {
  console.error('❌ SSH: Chrome API not available');
} else {
  console.log('✅ SSH: Chrome API available');
}
...
class HighlightManager {
  ...
  init() {
    // Setup message listener before everything else
      ...
      try {
        ...
        if (message.action === 'ping') {
          if (sendResponse && typeof sendResponse === 'function') {
            sendResponse({
              status: 'alive',
              loadTime: window.superHighlighterLoadTime,
              timestamp: Date.now()
            });
          }
          return true;
  ...
  setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      try {
        if (e.ctrlKey && e.shiftKey && this.isEnabled) {
          ...
          // Skip if it's just the Shift key itself or invalid key
    ...
    }, {passive: false}); // passive: false because we might preventDefault
  ...
  handleMessage(message, sendResponse) {
    ...
    if (message.action !== 'get-highlights') {
  ...
  highlightSelection(color) {
    ...
    if (!selection || selection.rangeCount === 0) {
      ...
      console.warn('⚠️ No stored selection available either');
      this.showTemporaryMessage('Lütfen önce bir metin seçin');
      return;
    }
    ...
    if (!selectedText || selectedText.length === 0) {
      ...
      this.showTemporaryMessage('Seçilen metin boş');
      return;
    }

    if (selectedText.length < 2) {
      console.warn('⚠️ Selected text too short');
      this.showTemporaryMessage('Lütfen daha uzun bir metin seçin');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Last resort: manual text replacement
          try {
            const span = document.createElement('span');
            span.className = `ssh-highlight ssh-${highlightData.color}`;
            span.dataset.highlightId = highlightData.id;
  ...
  removeHighlightFromSelection() {
    const selection = window.getSelection();
    if (!selection || selection.rangeCount === 0) {
      console.log('⚠️ No highlight found to remove - no selection');
      return;
    }
    ...
    // Search up the DOM tree for a highlight element
    let currentElement = highlightElement;
    while (currentElement && currentElement !== document.body) {
      if (currentElement.classList && currentElement.classList.contains('ssh-highlight')) {
        const highlightId = currentElement.dataset.highlightId;
        if (highlightId) {
          console.log('🗑️ Found highlight to remove:', highlightId);
          this.removeHighlightById(highlightId);
          return;
        }
      }
    ...
    // If no highlight found in selection, try to find any highlight near the cursor
    const clickedElement = document.elementFromPoint(range.getBoundingClientRect().x || 0, range.getBoundingClientRect().y || 0);
    if (clickedElement && clickedElement.classList.contains('ssh-highlight')) {
      const highlightId = clickedElement.dataset.highlightId;
      if (highlightId) {
        console.log('🗑️ Found nearby highlight to remove:', highlightId);
        this.removeHighlightById(highlightId);
        return;
      }
    }

    console.log('⚠️ No highlight found to remove - no highlight element found');
  }
  ...
  loadHighlights() {
    ...
    // Always load from storage first (most reliable)
    this.loadHighlightsFromStorage().finally(() => {
      this.isLoading = false;
    });
  }

  async loadHighlightsFromStorage() {
    try {
      // Check if chrome.storage is available
      if (!chrome || !chrome.storage || !chrome.storage.local) {
        console.error('❌ SSH: Chrome storage API not available');
        this.loadFromEmergencyBackup();
        this.isLoading = false;
        return;
      }
      ...
      if (result[urlKey]) {
        ...
        console.log('📥 SSH: Loaded from storage:', this.highlights.length, 'highlights');
        this.restoreHighlights();
      } else {
        this.highlights = [];
        console.log('📝 SSH: No highlights found in storage for this page');
        
        // Check for emergency backup
        this.loadFromEmergencyBackup();
      }
    } catch (storageError) {
      console.error('❌ SSH: Storage access failed:', storageError);
      
      if (storageError.message && storageError.message.includes('Extension context invalidated')) {
        console.log('🔄 SSH: Extension context invalidated during load, trying emergency backup...');
        this.loadFromEmergencyBackup();
  ...
  loadFromEmergencyBackup() {
    try {
      const urlKey = this.getUrlKey(window.location.href);
      const emergencyKey = 'ssh_emergency_' + urlKey;
      const backup = localStorage.getItem(emergencyKey);
      
      if (backup) {
        const backupData = JSON.parse(backup);
        if (backupData.highlights && Array.isArray(backupData.highlights)) {
          this.highlights = backupData.highlights;
          console.log('📥 SSH: Loaded from emergency backup:', this.highlights.length, 'highlights');
          this.restoreHighlights();
          return true;
        }
      }
    } catch (error) {
      console.error('❌ SSH: Emergency backup load failed:', error);
    }
    return false;
  }
  ...
  saveHighlights() {
    ...
    if (this.isSaving) {
      console.log('🔄 SSH: Save already in progress, skipping...');
      return;
    }
    ...
    this.saveTimeout = setTimeout(async () => {
      try {
        ...
        // Always save directly to storage for reliability
        await this.saveHighlightsToStorage();
        
        // Also try background script as secondary method (don't wait for it)
        if (chrome && chrome.runtime) {
          chrome.runtime.sendMessage({
            action: 'save-highlights',
            highlights: this.highlights
          }).catch(() => {
            // Ignore background script errors - we already saved to storage
            console.log('📝 SSH: Background script save failed, but storage save succeeded');
          });
        }
        
      } catch (error) {
        console.error('❌ SSH: Failed to save highlights:', error);
      } finally {
        this.isSaving = false;
      }
  ...
  async saveHighlightsToStorage() {
    try {
      ...
      const urlKey = this.getUrlKey(window.location.href);
      const saveData = {};
      saveData[urlKey] = this.highlights;
      
      await chrome.storage.local.set(saveData);
      console.log('💾 SSH: Highlights saved directly to storage:', this.highlights.length);
    } catch (storageError) {
      console.error('❌ SSH: Direct storage save failed:', storageError);
      
      // If storage fails due to context invalidation, try to reload extension context
      if (storageError.message && storageError.message.includes('Extension context invalidated')) {
        console.log('🔄 SSH: Extension context invalidated, attempting recovery...');
        this.handleContextInvalidation();
      }
    }
  }
  ...
  handleContextInvalidation() {
    console.log('🛠️ SSH: Handling extension context invalidation...');
    
    // Store highlights in local storage as emergency backup
    try {
      const urlKey = this.getUrlKey(window.location.href);
      const emergencyKey = 'ssh_emergency_' + urlKey;
      ...
      console.log('💾 SSH: Emergency backup stored in localStorage');
    } catch (e) {
      console.error('❌ SSH: Emergency backup failed:', e);
    }

    // Try to recover extension context by reloading the content script
    setTimeout(() => {
  ...
  async attemptContextRecovery() {
    console.log('🔄 SSH: Attempting context recovery...');
    
    try {
      // Try to check if Chrome APIs are working again
      if (chrome && chrome.storage && chrome.storage.local) {
        // Test with a simple operation
        await chrome.storage.local.get('test');
        console.log('✅ SSH: Extension context recovered!');
        
        // Try to restore from emergency backup if needed
        this.restoreFromEmergencyBackup();
        return true;
      }
    } catch (error) {
      console.warn('⚠️ SSH: Context recovery failed:', error);
    }
    
    // If still failing, show user message
  ...
  restoreFromEmergencyBackup() {
    try {
      ...
      if (backup) {
        ...
        if (backupData.highlights && Array.isArray(backupData.highlights)) {
          this.highlights = backupData.highlights;
          console.log('🔄 SSH: Restored from emergency backup:', this.highlights.length, 'highlights');
          
          // Try to save back to proper storage
          this.saveHighlights();
      ...
      console.error('❌ SSH: Emergency backup restore failed:', error);
    }
  }

  showContextInvalidationMessage() {
    // Show user-friendly message about extension context issue
    const messageEl = document.createElement('div');
    messageEl.style.cssText = `
      position: fixed;
      top: 20px;
    ...
    messageEl.innerHTML = `
      <div style="font-weight: bold; margin-bottom: 8px;">⚠️ Highlighter Extension Issue</div>
      <div style="margin-bottom: 8px;">Extension needs to be reloaded. Please:</div>
      <div style="font-size: 12px;">
        1. Go to chrome://extensions/<br>
        2. Click reload button on Super Highlighter<br>
        3. Refresh this page
      </div>
      <div style="margin-top: 10px; text-align: right;">
        <button onclick="this.parentElement.remove()" style="background: none; border: 1px solid white; color: white; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px;">Close</button>
      </div>
    `;
  ...
  restoreHighlights() {
    ...
    this.highlights.forEach((highlight, index) => {
      try {
        ...
        // Try to find and restore the highlight
        if (this.restoreSingleHighlight(highlight)) {
          successCount++;
        } else {
          errorCount++;
        }
    ...
    console.log(`✅ SSH: Restore completed - Success: ${successCount}, Errors: ${errorCount}`);
  }

  restoreSingleHighlight(highlight) {
    // Simple text search and restore
    const walker = document.createTreeWalker(
      document.body,
    ...
    while (textNode = walker.nextNode()) {
      ...
      const startIndex = nodeText.indexOf(highlight.text);
      ...
      if (startIndex !== -1) {
        try {
          ...
          if (rangeText === highlight.text || 
              rangeText.replace(/\s+/g, ' ').trim() === highlight.text.replace(/\s+/g, ' ').trim()) {
            
            return this.applyHighlight(range, highlight);
          }
        } catch (rangeError) {
          console.warn('Range creation failed for highlight:', rangeError);
          continue;
        }
      }
    }
    
    console.warn('Could not find text to restore:', highlight.text.substring(0, 30) + '...');
    return false;
  }
