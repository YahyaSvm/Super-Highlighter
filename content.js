...
class HighlightManager {
  ...
  applyHighlight(range, highlightData) {
    try {
      ...
      // Try to find the text in the current DOM
      const textToFind = highlightData.text;
      const xpath = highlightData.xpath;
      
      // First try to use the original range if it's valid
      if (range && range.startContainer && range.endContainer) {
        try {
          const rangeText = range.toString();
          const normalizedRangeText = rangeText.replace(/\s+/g, ' ').trim();
          const normalizedTargetText = textToFind.replace(/\s+/g, ' ').trim();
          
          if (rangeText === textToFind || normalizedRangeText === normalizedTargetText) {
            ...
            console.log('✅ Highlight applied successfully with original range');
            return true;
          }
        } catch (rangeError) {
          console.warn('⚠️ Original range is invalid:', rangeError);
        }
      }
      
      // If original range doesn't work, try to find the text using xpath
      ...
      if (element) {
        ...
        if (startIndex !== -1) {
          ...
          if (textNode) {
            ...
            newRange.setEnd(textNode, startIndex + textToFind.length);
            ...
            console.log('✅ Highlight applied successfully with xpath fallback');
            return true;
          }
        }
      }
      
      console.warn('⚠️ Could not find text to highlight, trying fallback method');
      throw new Error('Range not found');
      
    } catch (e) {
      console.log('🔄 Primary method failed, trying surroundContents fallback...');
      
      // Fallback method: try surroundContents if range is still valid
      if (range && range.startContainer && range.endContainer) {
        try {
          const span = document.createElement('span');
          span.className = `ssh-highlight ssh-${highlightData.color}`;
          ...
          console.log('🔄 SurroundContents failed, trying manual method...');
        }
      }
      
      // Last resort: manual wrapping
      try {
        ...
        console.error('❌ All highlight methods failed:', manualError);
        return false;
      }
    }
